  ...
  return (
    <>
      <section className="border-b border-[var(--rule)]">
        <div className="mx-auto max-w-5xl px-6 py-24 md:px-10">
          ...
          >
            ...
            <span className="italic text-[color:var(--blue-ink)]">data</span>,{" "}
            <span className="italic text-[color:var(--teal-deep)]">medicine</span>, and the{" "}
            <span className="italic text-[color:var(--blue-ink)]">policies</span>{" "}
            that move them.
          </motion.h1>
          <div className="mt-12 grid gap-10 md:grid-cols-[1fr_1fr]">
            <p className="font-display text-lg leading-relaxed text-foreground/80 md:text-xl">
      ...
      <section className="border-b border-[var(--rule)] bg-[color:var(--paper-warm)]/40">
        <div className="mx-auto grid max-w-5xl gap-10 px-6 py-24 md:grid-cols-[1fr_2fr] md:px-10">
          <SectionLabel index="02">Timeline</SectionLabel>
          <ul className="space-y-8">
            {timeline.map((t) => (
              <li key={t.what} className="grid gap-1 border-l border-[var(--rule)] pl-6">
                <span className="text-[10px] font-semibold uppercase tracking-[0.25em] text-[color:var(--teal-deep)]">
                  {t.when}
                </span>
                <p className="font-display text-2xl">{t.what}</p>
                <p className="text-sm italic text-[color:var(--muted-ink)]">{t.where}</p>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="mx-auto grid max-w-5xl gap-10 px-6 py-24 md:grid-cols-[1fr_2fr] md:px-10">
          ...
          <div className="grid gap-8 sm:grid-cols-2">
            {Object.entries(skills).map(([group, items]) => (
              <div key={group}>
                <p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-[color:var(--muted-ink)]">
                  {group}
                </p>
                <ul className="mt-3 flex flex-wrap gap-2">
                  {items.map((s) => (
                    <li
                      key={s}
                      className="border border-[var(--rule)] bg-white px-3 py-1.5 text-sm"
                    >
                      {s}
                    </li>
                  ))}
